...
class Report:
    ...
    def write_Report(self, key, value):
        # 将新的key和value放在字典最前方
    ...
    def read_Report(self):
        import report.result as result
        reload(result)
        # print(reslut.report)
...
class Java:
    def __init__(self, src):
        ...
        self.test = 'defects4j test -r' + path  # 执行所有测试用例
        # test_all.txt  所有测试用例
        # failing_tests.txt 失败测试用例
        self.coverage_a = 'defects4j coverage -r' + path
        self.coverage_t = 'defects4j coverage -t %s' + path
        # coverage.xml 执行信息

        self.timeout = True # 计算跳出时间

    def program_start(self):
        self.timeout = False
        self.sys_test()
        self.all()
        self.tests_cov(self.test_reulut)
    
    # mbfl变异体执行
    def muti_start(self):
        try:
            self.sys_test()
            # 限制测试时间，生成faile all文件
            self.all()
            # 获取原始程序全测试用例执行结果
            return True
        except:
    ...
    # 执行所有测试用例，获取所有测试用例信息及失败测试用例信息
    def sys_test(self):
        if self.timeout:
            st = time.time()
            # print(self.test)
            os.system(self.test)
            self.checktime(st, time.time())
        else:
            os.system(self.test)
        self.timeout = False
    ...
    def all(self):
        ...
        atest = []
        ftest = []
        reuluts = self.test_reulut
        with open(os.path.join(self.src, 'all_tests'), 'r') as f:
            for lines in f.readlines():
                classf = self.functionclass2classfunction(lines)
                if classf in reuluts:
                    reuluts[classf].append(1)
                else:
                    reuluts[classf] = [1]
                atest.append(classf)
        ...
        # 读取失败测试用例信息
        with open(os.path.join(self.src, 'failing_tests'), 'r') as f:
            for lines in f.readlines():
                if '---' not in lines:
                    continue
                _, cf = lines.split()
        ...
        self.alltest = atest
        self.failtest = ftest
        self.test_reulut = reuluts
            # return True
        # except:
            # return False

    # 执行完第一次test后对后续的test添加时间限制
    def checktime(self, st, et):
        timelimit = int((et - st) * 5)
        self.test = 'timeout %s ' % timelimit + self.test
        # print(self.test)

    # 将test_all中的function class的测试用例转换为 class :: function
    def functionclass2classfunction(self, fc):
        [f, c] = fc.replace('(', ' ').replace(')', '').replace('\n', '').split()
        return c + '::' + f
    
    # 获取失败测试用例覆盖信息
    def failtest_cov(self, testcase):
        pass

    # 对每个测试用例执行覆盖
    def tests_cov(self, testcase):
        # for num, cf in enumerate(self.test_reulut):
        for num, cf in enumerate(testcase):
            self.single_cov(cf)
            print(self.cov_info)
    ...
    def xml_read(self, xml_path):
        try:
            cov_info_restore = self.cov_info
            elemen_obj = xmldom.parse(xml_path).documentElement

            # 循环packages
            element_obj_package_s = elemen_obj.getElementsByTagName('package')
            for element_obj_package in element_obj_package_s:
                package_name = element_obj_package.getAttribute('name')
                if package_name not in cov_info_restore:
                    cov_info_restore[package_name] = {}

                # 循环class
                element_obj_class_s = element_obj_package.getElementsByTagName('class')
                for element_obj_class in element_obj_class_s:
                    class_name = element_obj_class.getAttribute('filename')
                    if class_name not in cov_info_restore[package_name]:
                        cov_info_restore[package_name][class_name] = {}

                    # 循环line
                    element_obj_line_s = element_obj_class.getElementsByTagName('line')
                    hitline = []
                    for element_obj_line in element_obj_line_s:
                        line = element_obj_line.getAttribute('number')
                        if line not in hitline:
                            hitline.append(line)
                        else:
                            continue
                        hits = int(element_obj_line.getAttribute('hits'))
                        if line not in cov_info_restore[package_name][class_name]:
                            cov_info_restore[package_name][class_name][line] = [hits]
                        else:
                            cov_info_restore[package_name][class_name][line].append(int(hits))
            self.cov_info = cov_info_restore
        except:
            for package_name in self.cov_info:
                for class_name in self.cov_info[package_name]:
                    for line in self.cov_info[package_name][class_name]:
                        self.cov_info[package_name][class_name][line].append(-1)
        return True

    # 读取xml文件
    def xml_new_read(self, xml_path):
        try:
            cov_info_restore = {}
            elemen_obj = xmldom.parse(xml_path).documentElement
            ...
            for package_name in self.cov_info:
                for class_name in self.cov_info[package_name]:
                    for line in self.cov_info[package_name][class_name]:
                        self.cov_info[package_name][class_name][line].append(-1)
        return cov_info_restore

    # 单个测试用例覆盖信息
    def single_cov(self, test):
        # print(self.coverage_t % test)
        os.system(self.coverage_t % test)
        self.xml_read(os.path.join(self.src, 'coverage.xml'))
