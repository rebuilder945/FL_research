...
from DNMBFL.util import File
import DNMBFL.util
# import DNMBFL.sbfl.sbfl_for as sbfl_for
from DNMBFL.mbfl.mutpolyn import mutation_trick
from DNMBFL.mbfl.mutpolyn import new_mutation_trick_python
from DNMBFL.mbfl.mutpolyn import java_mutation
...
def rad(l_min, l_max, num):
    rad_list = []
    i = 0
    while i:
        cache = random.randint(l_min, l_max)
...
class Fom:
    ...
    def fom_data_old(self, fompath, program_path, cov_info=[]):
        fom_record = ''
        lines = File().read_line(program_path)
    ...
    def fom_data(self, fompath, program_path, cov_info=[]):
        """
        param: fompath: fom保存路径
        param: program_path: 学生代码路径
        param: cov_info: 错误测试覆盖行，从1开始
        """
        fom_record = ''
        lines = File().read_line(program_path)
    ...
    def hom_data(self, fom_path):
        ...
        #  全排列生成
        # for fommessage1 in lines:
        #     for fommessage2 in lines:
        #         me1 = eval(fommessage1)[0]
        #         me2 = eval(fommessage2)[0]
        #         if not me1[0] == me2[0]:
        #             fom_record += str([me1, me2]) + '\n'
    ...
    # 新算法构造一阶变异体
    def fomCreatNewop(self, line: str):
        fom_messages = []
        for operator_dict in new_mutation_trick_python:
    ...
    def mutation_build(self, program_path, mutation_path, mutation_message):
        ...
        :param program_path: 原代码路径
        :param mutation_path: 生成的变异体路径
        :param mutation_message: 变异信息，字典，包含错误覆盖行和对应的变异规则
        """
        
        try:
            # mutation_message = [[7, '     int a, p = 0, i;', '     short int a, p = 0, i;', 'int', 'short int', 5],
            #                     [7, '     int a, p = 0, i;', '     short int a, p = 1, i;', '0', '1', 16]]
            lines = File().read_line(program_path)
    ...
    def mutation_work(self, program_path, testdata_dirpath):
        # 读取变异信息
        fom_text_original = File().read_line(r'./report/Fom-run.txt') if globalmessage else File().read_line(
        ...
        for text in fom_text_original:
            ...
            File().remove_relative(fom_path, True)
...
def test():
    result_src = [True, False, False, False, False, False, False, True]
    result_fom = {
    ...
    # print(get_toule(result_src, result_fom))


def main(mainmessage):
    globalmessage = mainmessage
    fom = Fom()
    if not globalmessage['mbfl']['fomdata']:
        fom.fom_data(mainmessage['program_path'], mainmessage['execute'])
        # 根据第二个参数信息生成变异体
    result = fom.mutation_work(mainmessage['program_path'], mainmessage['testdata_dirpath'])
    with open('report/mbfl-fom.txt', 'w') as f:
        f.write(str(result))
    mainmessage = suspicious(mainmessage, result)
    return mainmessage
