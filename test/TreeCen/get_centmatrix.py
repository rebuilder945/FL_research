...
def getnodeandedge_(node, src, tgt):
    for child in node.children:
        src.append(node.type)
        tgt.append(child.type)
        getnodeandedge_(child, src, tgt)
...
def get_single_cent_matrix(path):
    ...
    #     # print(programtext) 即代码文本内容
    #     tokens = list(javalang.tokenizer.tokenize(programtext))
    # # print(tokens) 即代码中每个词和位置：[Separator "}" line 12, position 5, Separator "}" line 13, position 1]
    # # try:
    ...
    with open(path, 'r', encoding='utf-8') as src_file:
        code_str = src_file.read()
    # tree-sitter config
    PY_Lang = Language(tspython.language())
    py_parser = Parser()
    py_parser.language = PY_Lang
    newtree = py_parser.parse(bytes(code_str, 'utf8'))

    src = []
    tgt = []
    # getnodeandedge(newtree, src, tgt)
    getnodeandedge_(newtree.root_node, src, tgt)

    all_nodes = set(src + tgt)

    type2index = {}
    for index, type in enumerate(all_nodes):
        type2index[type] = index
        g.add_node(index, name = type)
    ...
    # except:
    #     return
    for _, l in this_all_cents.items():
        print(len(l))
...
def main():
    # print(1111)
    src_path = 'test/TreeCen/test_data_py/'
    out_path = 'test/TreeCen/output_.json'
    # javalist = glob.glob(os.path.join(java_src_path, '*.java'))
    files = os.listdir(src_path)
    # print(javalist)
    all_cent_mtxs = dict()
    for file in tqdm(files):
        file_path = os.path.join(src_path, file)
        file_id = file.rsplit("/")[-1][:-5]
        # print(215)
        cent_metrix_dict = get_single_cent_matrix(file_path)
