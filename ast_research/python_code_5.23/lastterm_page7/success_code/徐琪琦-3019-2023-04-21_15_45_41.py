...
s = input().split()
dic = {}
dic[name] = dic.get(name,s[0])
dic[english] = dic.get(engilsh,s[1])
dic[python] = dic.get(python,s[2])
dic[math] = dic.get(math,s[3])
dic[avg] = dic.get(avg,(dic[english]+dic[python]+dic[math])/3)
dic.sort(key = lambda x:x[1])
print(dic[name],dic,dic[avg])
