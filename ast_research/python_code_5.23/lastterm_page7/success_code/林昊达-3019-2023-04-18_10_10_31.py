...
stu["english"]=float(stu["english"])
stu["python"]=float(stu["python"])
stu["math"]=float(stu["math"])
stu["avg"]=(stu["english"]+stu["python"]+stu["math"])/3
ls=[stu["english"],stu["python"],stu["math"]]
ls.sort(reverse=True)
print(stu["name"]+" "+"{:.2f}".format(ls[0])+" "+"{:.2f}".format(ls[1])+" "+"{:.2f}".format(ls[2])+" "+"{:.2f}".format(stu["avg"]))
