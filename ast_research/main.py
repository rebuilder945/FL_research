...
cluster_range = (40, 200) # kmeans聚类中心范围
dis_threshold = 1 # 树22匹配距离认为相似的阈值
cls_flag = False # 错误还是正确的程序中进行聚类
...
kmeans_output_path = os.path.join(
    f"ast_research/out/kmeans_output/",
    code_from,
...
match22_ouput_path = os.path.join(
    ...
    code_from,
    f"{cls_flag}_{depth_range[0]}_{depth_range[1]}-{dis_threshold}"
)
